...
// newAccessControlPolicy returns a AccessControlPolicy object
// with each the field set by the resource's corresponding spec field.
func (rm *resourceManager) newAccessControlPolicy(
	r *resource,
) *svcsdk.AccessControlPolicy {
	res := &svcsdk.AccessControlPolicy{}

	if r.ko.Spec.AccessControlPolicy.Grants != nil {
		resf0 := []*svcsdk.Grant{}
		for _, resf0iter := range r.ko.Spec.AccessControlPolicy.Grants {
			resf0elem := &svcsdk.Grant{}
			if resf0iter.Grantee != nil {
				resf0elemf0 := &svcsdk.Grantee{}
				if resf0iter.Grantee.DisplayName != nil {
					resf0elemf0.SetDisplayName(*resf0iter.Grantee.DisplayName)
				}
				if resf0iter.Grantee.EmailAddress != nil {
					resf0elemf0.SetEmailAddress(*resf0iter.Grantee.EmailAddress)
				}
				if resf0iter.Grantee.ID != nil {
					resf0elemf0.SetID(*resf0iter.Grantee.ID)
				}
				if resf0iter.Grantee.Type != nil {
					resf0elemf0.SetType(*resf0iter.Grantee.Type)
				}
				if resf0iter.Grantee.URI != nil {
					resf0elemf0.SetURI(*resf0iter.Grantee.URI)
				}
				resf0elem.SetGrantee(resf0elemf0)
			}
			if resf0iter.Permission != nil {
				resf0elem.SetPermission(*resf0iter.Permission)
			}
			resf0 = append(resf0, resf0elem)
		}
		res.SetGrants(resf0)
	}
	if r.ko.Spec.AccessControlPolicy.Owner != nil {
		resf1 := &svcsdk.Owner{}
		if r.ko.Spec.AccessControlPolicy.Owner.DisplayName != nil {
			resf1.SetDisplayName(*r.ko.Spec.AccessControlPolicy.Owner.DisplayName)
		}
		if r.ko.Spec.AccessControlPolicy.Owner.ID != nil {
			resf1.SetID(*r.ko.Spec.AccessControlPolicy.Owner.ID)
		}
		res.SetOwner(resf1)
	}
...
// setResourceAccessControlPolicy sets the `AccessControlPolicy` spec field
// given the output of a `GetBucketAcl` operation.
func (rm *resourceManager) setResourceAccessControlPolicy(
	r *resource,
	resp *svcsdk.GetBucketAclOutput,
) *svcapitypes.AccessControlPolicy {
	res := &svcapitypes.AccessControlPolicy{}
	if resp.Grants != nil {
		resf0 := []*svcapitypes.Grant{}
		for _, resf0iter := range resp.Grants {
			resf0elem := &svcapitypes.Grant{}
			if resf0iter.Grantee != nil {
				resf0elemf0 := &svcapitypes.Grantee{}
				if resf0iter.Grantee.DisplayName != nil {
					resf0elemf0.DisplayName = resf0iter.Grantee.DisplayName
				}
				if resf0iter.Grantee.EmailAddress != nil {
					resf0elemf0.EmailAddress = resf0iter.Grantee.EmailAddress
				}
				if resf0iter.Grantee.ID != nil {
					resf0elemf0.ID = resf0iter.Grantee.ID
				}
				if resf0iter.Grantee.Type != nil {
					resf0elemf0.Type = resf0iter.Grantee.Type
				}
				if resf0iter.Grantee.URI != nil {
					resf0elemf0.URI = resf0iter.Grantee.URI
				}
				resf0elem.Grantee = resf0elemf0
			}
			if resf0iter.Permission != nil {
				resf0elem.Permission = resf0iter.Permission
			}
			resf0 = append(resf0, resf0elem)
		}
		res.Grants = resf0
	...
	if resp.Owner != nil {
		resf1 := &svcapitypes.Owner{}
		if resp.Owner.DisplayName != nil {
			resf1.DisplayName = resp.Owner.DisplayName
		}
		if resp.Owner.ID != nil {
			resf1.ID = resp.Owner.ID
		}
		res.Owner = resf1
	}
